...
\rkk{An overview of pneumatic and hydraulic microactuators has been provided by Volder et al. \cite{volder2010pneumatic}.} 
\rkk{Ikuta and Suzuki \cite{ikuta2002safety} developed a multiple-segment micro-hydraulic actuator that can be controlled open-loop within blood vessels.}
\rkk{Xiao and Vatcha\cite{xiao2010realtime} present simulation results of an online motion planner for a planar continuum manipulators.}
\rkk{ Li and Xiao \cite{li2015general} extend this work to present a more general formulation to constrained, continuum manipulation.}
There are \rkk{also} several examples of soft grippers described in recent literature, we will mainly focus on fluidic-based systems.
...
The work presented in this work builds on top \rkk{of the previous} system and expands its capabilities \rkk{to do actual manipulation}.
The previous version \rkk{was only suitable for moving a soft manipulator through a constrained environment without object interaction or manipulation.
That system is only suitable for inspection tasks}.
...
\begin{itemize}
  ...
  \item Repeatable successful grasping demonstrations with a physical prototype and a \rkk{qualitative} experimental characterization of the uncertainty regions that can be tolerated by the soft gripper.
  %\item End-effector control of the grasping manipulator attached to a soft arm using a motion tracking system.
  \item \rkk{Demonstrating that soft robots allow for proper autonomous manipulation of various delicate objects without requiring force sensing or accurate positioning.} 
\end{itemize}
